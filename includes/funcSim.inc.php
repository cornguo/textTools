...
function checkDist($sentence, $kw, $query)
{
	...
	$sign = 1;
	if(mb_stripos($query, $kw, 0, 'UTF-8') > 0) $sign = -1;
	$kwPos = mb_stripos($sentence, $kw, 0, 'UTF-8');
	if($kwPos === FALSE) return 99;
	$apStr = str_replace($kw, "", $query);
	if(strlen($apStr) == 0) return 0;
	$apPos = mb_stripos($sentence, $apStr, 0, 'UTF-8');
	if($apPos === FALSE) return 99;
...
function biSim($sent1, $sent2, $kw = null, $query = null, $method = "jaccard")
{
	if($kw != null)
	{
		...
		$query = str_replace("\"", "", $query);
		$apStr = str_replace($kw, "", $query);
		//$sent1 = str_replace($kw, ",", $sent1);
		//$sent2 = str_replace($kw, ",", $sent2);
		$pattern = array(
			"/{$kw}.?.?{$apStr}/Uui",
			"/{$apStr}.?.?{$kw}/Uui"
		);
...
//print_r($bi1);
//print_r($bi2);
	if(count($bi1) == 0 || count($bi2) == 0) return -1;
