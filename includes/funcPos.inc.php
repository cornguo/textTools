...
function stripPOS($POSText) {
    if (is_array($POSText)) {
        ...
        foreach ($POSText['text'] as $tokN => $token) {
            $output .= $token;
            if (
                isset($POSText['text'][$tokN+1]) && (
                    ('FW' === $POSText['tag'][$tokN] || 'Neu' === $POSText['tag'][$tokN])
                    && ('FW' === $POSText['tag'][$tokN+1] || 'Neu' === $POSText['tag'][$tokN+1])
                )
            ) {
                $output .= ' ';
            }
...
function getBIOPosArr($POSArr, $normText = true) {
    ...
    foreach ($POSArr['text'] as $tokN => $tokData) {
        ...
        foreach ($sCharList[0] as $sN => $sChar) {
            if (true === $normText) {
                $outArr['text'][$charN] = normText($sChar);
            } else {
                $outArr['text'][$charN] = $sChar;
            }
//            echo "text: [{$sChar}], norm: [{$outArr['text'][$charN]}], {$tokTag}\n";
            if (0 === $sN) {
                $outArr['tag'][$charN] = "B-{$tokTag}";
            } else {
                $outArr['tag'][$charN] = "I-{$tokTag}";
            }
            $charN++;
        }
    }
...
function POSArray2Text($posArr) {
    ...
    if (!isset($posArr['text'])) {
        foreach ($posArr as $sentData) {
            foreach ($sentData['text'] as $segN => $segT) {
                $output .= "{$segT}({$sentData['tag'][$segN]}) ";
            }
            $output = rtrim($output) . "\n";
        }
    } else {
        foreach ($posArr['text'] as $segN => $segT) {
            $output .= "{$segT}({$posArr['tag'][$segN]}) ";
        }
    }
    return trim($output);
}
...
function segArr2Text($segArr) {
    ...
    for($loop = 0; $loop < count($segArr['tokens']['text']); $loop++) {
        ...
        if ($loop+1 < count($segArr['tokens']['text'])) {
            $output .= ' ';
        }
    }
...
function getPOSArray($text) {
    ...
    if (is_array($text)) {
        foreach ($text as $n => $line) {
            $output[$n] = POS2Array($line);
        }
    } else {
        $output[0] = POS2Array($text);
    }
...
//   echo "\n";
    ...
    for($tokenN = 0; $tokenN < count($POSArr['text']); $tokenN++) {
        ...
        while(false !== strstr($focus, $token)) {
            if ($match['start'] == -1) {
                $match['start'] = $tokenN;
            }
            $match['text'] .= $token;
            $tokenN++;
            if ($tokenN < count($POSArr['text'])) {
                $token = $POSArr['text'][$tokenN];
            } else {
                break;
            }
