...
function isEngWord($word) {
    if (preg_match('/[\p{Ll}|\p{Lm}|\p{Lt}|\p{Lu}|\p{N}|\p{Pd}|\.]/ui', $word)) {
        return true;
    }
    return false;
}
...
//        debug("bigram:: getBigramArray: only one item in array, return as intact");
    ...
    for ($i = 0; $i < count($arr); $i++) {
        if (isEngWord($arr[$i])) {
            $outArr[] = $arr[$i];
        } elseif (isset($arr[$i+1]) && !isEngWord($arr[$i+1])) {
            if (0 === $i && ',' === $arr[$i+1]) {
                $outArr[] = $arr[0];
            }
            if (',' === $arr[$i] && ((isset($arr[$i+2]) && ',' === $arr[$i+2]) || !isset($arr[$i+2]))) {
                $outArr[] = $arr[$i+1];
            } elseif ($arr[$i] != ',' && $arr[$i+1] != ',') {
                $outArr[] = "{$arr[$i]}{$arr[$i+1]}";
            }
        }
    }
/*    if (DEBUG) {
...
function getStringArray($string) {
    ...
    while(0 !== mb_strlen($string, 'UTF-8')) {
        $garbage = array('_', '[', ']', '　');
